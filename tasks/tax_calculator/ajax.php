...
function pph21($C,$T,$npwp){
    ...
    if($initial_tax['bracket']==$next_tax['bracket']){
        ...
        $dpp=$gross_up*0.5; 
        $tax=ceil($dpp*($initial_tax['percent']/100));
        ...
        return $return;
           ...
           if($next_tax['bracket']==2){ 
            ...
            // echo (120000000-$C)*0.5*tax_bracket(1,$npwp)/100;die;
            $cal1=($T+(120000000-$C)*0.5*tax_bracket(1,$npwp)/100
            -(120000000*0.5*tax_bracket(2,$npwp)/100)+$C*0.5*tax_bracket(2,$npwp)/100);
            
          $cal2=(1-(tax_bracket(2,$npwp)/100*0.5));
...
function pph23($T){
    ...
    $tax=$dpp*0.02;   
    $gross_up-$tax;
    $return['invoice_groupm']=ceil($gross_up);
    $return['kol_payment']=ceil($gross_up-$tax);
    $return['tax']=ceil($tax);
    return $return;
...
if($_SERVER['REQUEST_METHOD']=="POST"){
    if($_POST['type']==1){
        $return=pph21($_POST['c'],$_POST['t'],1);
    }else if($_POST['type']==2){
        $return=pph21($_POST['c'],$_POST['t'],0);
    }else if($_POST['type']==4){
        $return=pph23($_POST['t']);
    }else if ($_POST['type']==3){
        $return=pph23($_POST['t']);
        $ppn=ppn($_POST['t']);
        $return['ppn']=$ppn;
        $return['tax']=$return['tax'];
        $return['kol_payment']=$return['invoice_groupm']+$ppn-$return['tax'];
    }
